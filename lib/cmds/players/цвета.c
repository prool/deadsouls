...
int cmd() {
    write(
            ...
            "Специальные метки: %%^^BOLD%%^^ и %%^^FLASH%%^^ и %%^^RESET%%^^\n\n"
            "Их можно смешивать и совмещать, например: \n"
...
string GetHelp() {
    return ("Синтаксис: цвета\n\n"
            "Перечисляет все возможные цвета в соответствующем цвете.");
}
