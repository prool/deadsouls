...
mixed cmd(string args) {
    ...
    if(!args){ 
        this_player()->eventPrint("Синтаксис: <вспомнить [канал | сказать]>");
        return 1;
    }

    this_player()->eventPrint("Вспоминание...\n");

    if(!strsrch(args, "сказать")){
        load_object(CMD_TELL)->cmd("вспомнить");
        return 1;
    }
    ...
    if(talks && member_array(gfile, talks) != -1){
        string ret2 = "Ваша история канала"+gfile+": \n\n";
    ...
    if(!CHAT_D->CanListen(this_player(), gfile)){
        return "У вас нет доступа к: "+gfile;
    }
    if(!CHAT_D->GetListening(this_player(), gfile)){
        return "Вы не подключены к каналу: "+gfile;
    }
    if(gfile == "admin"){
...
string GetHelp(){
    string ret = "Синтаксис: вспомнить <КАНАЛ> [номер]\n"
        "        вспомнить [ say | whisper | tell | yell | shout ]\n\n"
        "Дает вам возможность воспроизвести необходимые сообщения.\n";
    if(!LOG_REMOTE_CHANS){
